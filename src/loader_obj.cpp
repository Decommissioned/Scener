...
static string ExtractName(const string& file)
{
        size_t start1 = file.find_last_of('/') + 1;
        size_t start2 = file.find_last_of('\\') + 1;
        size_t start = start1 > start2 ? start1 : start2;
        size_t end = file.find_last_of('.');
        return file.substr(start, end - start);
}
Mesh LoadWavefrontObject(const string& file)
{
        Mesh data;
        std::vector<tinyobj::shape_t> shapes;
        std::vector<tinyobj::material_t> materials;
        string error = tinyobj::LoadObj(shapes, materials, file.c_str(), "./");

        if (!error.empty())
        {
                HandleError("Mesh " + file + ": " + error, ErrorCriticality::MEDIUM);
        }
